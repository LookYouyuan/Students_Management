...
while True:
    ...
    if choice == 1:
        ...
        if add_choice == "a":
            ...
            id = input("请给课程指定学生的学号：")
            is_has = stu_management_system.is_has(id)
            if is_has != 0:

                courName = input("请输入课程名：")
                score = int(input("请输入分数："))
                is_has.addCourseScore(courName, score)
                while True:
                    choice = input("若不想继续添加课程，请输入q Q,若想继续添加，请输入y Y:")
